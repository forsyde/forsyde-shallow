...
data State   = Operating 
	     | Locked deriving(Eq, Show, Typeable)
$(deriveLift1 ''State)
...
buttonControl overrides bassDn bassUp trebleDn trebleUp 
    = (bass, treble) 
      where (bass, treble) = unzipSY levels
 	    levels = ((holdSY "buttonHold" (0.0, 0.0)) `funComb2` levelControl) 
                           button overrides
...
buttonInterface bassUp bassDn trebleUp trebleDn 
   = zipWith4SY "interpretButtons" f bassUp bassDn trebleUp trebleDn
     where f = $(newProcFun 
                  [d| f :: AbstExt Sensor ->
                           AbstExt Sensor ->
                           AbstExt Sensor ->
                           ...
                           AbstExt Button 
                      f bu bd tu td = 
                          if isPresent bu then Prst BassUp
                          else if isPresent bd then Prst BassDn
                          else if isPresent tu then Prst TrebleUp
                          else if isPresent td then Prst TrebleDn
                          else Abst |] )
\end{code}
...
levelControl button overrides 
  = mealy2SY "levelControl" nextState output (initState, initLevel) button overrides

nextState :: ProcFun ((State,(Double,Double)) -> AbstExt Button 
                      -> AbstExt OverrideMsg -> (State,(Double,Double)))
nextState = $(newProcFun
 [d| nextState :: (State,(Double,Double)) -> AbstExt Button 
               -> AbstExt OverrideMsg -> (State,(Double,Double))
...
output :: ProcFun ((a, (Bass, Treble)) -> AbstExt Button -> AbstExt OverrideMsg -> AbstExt (Bass,Treble))
output = $(newProcFun
  [d| output :: (a, (Bass, Treble)) -> AbstExt Button -> AbstExt OverrideMsg 
 	        -> AbstExt (Bass,Treble)
