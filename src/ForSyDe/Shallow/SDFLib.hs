...
-- | The process constructor 'mapSDF' takes
-- the number of consumed (@c@) and produced (@p@) tokens and a
-- function @f@ that
-- operates on a list, and results in an SDF-process that takes an input
-- signal and results in an output signal
...
zipWithSDF :: (Int, Int) -> Int -> ([a] -> [b] -> [c]) -> Signal a -> Signal b -> Signal c                                              
    ...
    | otherwise    
         = if length produced_tokens == p then
           ...
           where consumed_tokens_as = fromSignal $ takeS c1 as
                 ...
                 produced_tokens = f consumed_tokens_as
                                     consumed_tokens_bs consumed_tokens_cs
  

-- | The process constructor 'zipWith4SDF' takes a tuple @(c1, c2, c3,
-- c4)@ 
-- denoting the number of consumed tokens and an integer @p@ denoting
-- the number of produced tokens and a function @f@
-- that operates on three lists, and results in an SDF-process that takes three
    ...
    | otherwise    
         = if length produced_tokens == p then
           ...
           where consumed_tokens_as = fromSignal $ takeS c1 as
                 ...
                 produced_tokens 
                      = f consumed_tokens_as consumed_tokens_bs
...
actor32SDF (c1, c2, c3) (p1, p2) f as bs cs = unzipSDF (p1, p2) $ zipWith3SDF (c1, c2, c3) 1 f as bs cs
...
actor23SDF (c1, c2) (p1, p2, p3) f xs ys = unzip3SDF (p1, p2, p3) $ zipWithSDF (c1, c2) 1 f xs ys
...
actor33SDF (c1, c2, c3) (p1, p2, p3) f as bs cs = unzip3SDF (p1, p2, p3) $ zipWith3SDF (c1, c2, c3) 1 f as bs cs
...
actor24SDF (c1, c2) (p1, p2, p3, p4) f xs ys = unzip4SDF (p1, p2, p3, p4) $ zipWithSDF (c1, c2) 1 f xs ys
...
unzipSDF (p1, p2) xs = (s1, s2) 
  where s1 = signal $ f1 xs
        s2 = signal $ f2 xs
        f1 NullS         = []
        f1 ((as, _):-xs) = if length as == p1 then 
                             as ++ f1 xs
                           else  
                             error "unzipSDF: Process does not produce correct number of tokens"
        f2 NullS         = []
        f2 ((_, bs):-xs) = if length bs == p2 then 
                             bs ++ f2 xs
                           else  
                             error "unzipSDF: Process does not produce correct number of tokens"  
