...
selMapSY seed f0 f1 xs = selmap1 f0 f1 (sigmaUn seed (0,1)) xs
  where
    ...
    selmap1 f0 f1 (s:-_)     (x:-NullS) 
      = (select1 s f0 f1 x) :- NullS
    selmap1 f0 f1 (s:-NullS) (x:-_) 
      = (select1 s f0 f1 x) :- NullS
...
select1 s _ _ _   = error ("select1: seed value neither 0 nor 1: " ++ (show s))

select2 :: Int -> (a -> b -> c) -> (a->b->c) 
        -> a -> b -> c
select2 0 f0 _ x y = f0 x y
select2 1 _ f1 x y = f1 x y
select2 s _ _ _ _  = error ("select2: seed value neither 0 nor 1: " ++ (show s))
...
sigmaUn seed range = signal (stoch range (mkStdGen seed))
  where
    ...
    stoch range g = newNum `seq` (newNum : (stoch range newGen))
