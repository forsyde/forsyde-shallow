...
simulateDyn pSysDef inps | seq argsl True = runST (
  do -- List where to store the Vars generated by delay processes
     roots <- newSTRef []
         
     let sysF = (sysFun . readURef . unPrimSysDef) pSysDef
           ...
           case (nodeVarPairs,childVars) of
            ...
            (nodeVarPairs,
             Proc _ (SysIns pSysDef ins)) ->
               ...
               do let sysF = (sysFun . readURef . unPrimSysDef) pSysDef
                      iface = (iIface . readURef . unPrimSysDef) pSysDef
                  sr  <- traverseST 
                           (newInstance ins)
                           define 
                           (Netlist $ sysF (iface2InPorts iface))
